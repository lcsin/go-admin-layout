            ...
            <span>
              ...
              <span v-if="rename.status && rename.node.id === row.id">
                <input
                  ...
                  placeholder="请输入目录名称"
                  type="text"
                  @blur.stop="handleBlur(node, row)"
                  @keyup.enter="handleBlur(node, row)"
                >
              </span>
